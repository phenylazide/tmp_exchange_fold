...
OMP_NUM_THREADS: 2
	...
	pat4a = re.compile('gpu_OMP_NUM_THREADS: *(\d+)')
	pat4b = re.compile('gpu_ntmpi: *(\d+)')
	
	#pattern5 = re.compile('OMP_NUM_THREADS: *(\d+)')  # 	
	match3 = re.findall(pattern3,txt)
	#match4 = re.findall(pattern4,txt)
	mat4a = re.findall(pat4a,txt); mat4b = re.findall(pat4b,txt)
	OMP_NUM_THREADS = mat4a[0]; ntmpi = mat4b[0]
	...
	return read_reg_MM_steps,gpu_OMP_NUM_THREADS,gpu_ntmpi
...
def make_ndx_and_md_run(xx_run_fold=base):
	...
	read_reg_MM_steps,gpu_OMP_NUM_THREADS,gpu_ntmpi = read_reg_MM_steps_threads_from_parameters(xx_run_fold)
	...
	os.chdir(xx_run_fold)
	print('now running reg MM with gpu, gpu_OMP_NUM_THREADS and gpu_ntmpi is set to: {}'.format(str(gpu_OMP_NUM_THREADS),str(gpu_ntmpi)))
	bashline6 = 'export OMP_NUM_THREADS={};/opt/gmx2018.4/bin/gmx mdrun -v -deffnm md_run -ntmpi {} -pin on -nb gpu'.format(str(gpu_OMP_NUM_THREADS),str(gpu_ntmpi))
	os.system(bashline6)
